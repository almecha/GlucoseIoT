            ...
            if resource_type == "doctors":
                ...
                for i, doctor in enumerate(self.catalog["doctorsList"]):
                    if doctor["userID"] == resource_id:
                        ...
                        master_doctor = next((d for d in self.catalog["doctorsList"] if d["role"] == "MasterDoctor"),
                                             None)
                ...
                for i, patient in enumerate(self.catalog["patientsList"]):
                    if patient["userID"] == resource_id:
                        ...
                        doctor = next((d for d in self.catalog["doctorsList"] if d["userID"] == patient["doctorID"]),
                                      None)
                ...
                self.catalog["servicesList"] = [s for s in self.catalog["servicesList"] if
                                                s["serviceID"] != resource_id]
                deleted = len(self.catalog["servicesList"]) < len(
                    [s for s in self.catalog["servicesList"] if s["serviceID"] != resource_id])

            elif resource_type == "devices":
                self.catalog["devicesList"] = [d for d in self.catalog["devicesList"] if d["deviceID"] != resource_id]
                deleted = len(self.catalog["devicesList"]) < len(
                    [d for d in self.catalog["devicesList"] if d["deviceID"] != resource_id])
